...
/**
 * Live index of every loaded SBW drone (base `DroneEntity` and the Drone Warfare addon's
 * subclasses alike) per level — the [NpcRegistry] idea for a class we don't own, so it's fed from
...
object DroneRegistry {
    private val byLevel = IdentityHashMap<ServerLevel, LinkedHashSet<DroneEntity>>()

    @SubscribeEvent
    fun onJoin(event: EntityJoinLevelEvent) {
        val drone = event.entity as? DroneEntity ?: return
    ...
    fun all(level: ServerLevel): Collection<DroneEntity> = byLevel[level] ?: emptyList()
