...
class SquadManagerTest {
    ...
    fun `barracks replaces the class that actually died`() {
        ...
        assertEquals(listOf(MACHINE_GUNNER), missingClasses(composition, listOf(RIFLEMAN, RIFLEMAN, MEDIC)))
    }
    ...
    fun `unloaded members reserve a missing slot instead of being duplicated`() {
        ...
        assertEquals(1, missingClasses(composition, listOf(RIFLEMAN, null, MEDIC)).size)
    }
}
